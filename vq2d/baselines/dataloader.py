...
from .utils import extract_window_with_context, random_masking
...
class VisualQueryDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self.mask_ratio_reference   = mask_ratio_reference
        self.gt_rm_ratio            = gt_rm_ratio
        # fmt: on
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        ret = {
            ...
            "mask_ratio_reference": cfg.INPUT.VISUAL_CROP_MASK_RATIO,
            "gt_rm_ratio": cfg.INPUT.GT_RM_RATIO,
        }
    ...
    def __call__(self, dataset_dict):
        ...
        if os.stat(dataset_dict["file_name"]).st_size == 0:
            os.remove(dataset_dict["file_name"])
            dataset_dict["file_name"] = 'tmp.png'
        try:
        ...
        # load text
        ref_title = ref_dict["annotations"][0]["textual_name"]
        dataset_dict['title'] = regex.sub('', ref_title)
        ...
        # apply random mask
        if self.mask_ratio_reference>0 and self.is_train:
            reference = random_masking(reference, grid_size=32, 
                                       mask_ratio=self.mask_ratio_reference)
            # also apply masking on images
            if True:
                image_4d = dataset_dict["image"].unsqueeze(0)
                image_4d = random_masking(image_4d, grid_size=32, 
                                       mask_ratio=self.mask_ratio_reference)
                dataset_dict["image"] = image_4d.squeeze(0)
        dataset_dict["reference"] = reference.squeeze(0).byte()
        # apply random gt removal
        # self.gt_rm_ratio = 0.5 # TODO load from setting
        if self.gt_rm_ratio>0 and self.is_train:
            if random.random() < self.gt_rm_ratio:
                # print(' - dataloader: set gt rm to true')
                dataset_dict["gt_rm"] = True
            else:
                dataset_dict["gt_rm"] = False 
        else:
            dataset_dict["gt_rm"] = False 
